...
/// <summary>
/// This class imports .cube files and converts them into 3D textures to be used for color grading
/// </summary>
...
public class CubeLutImporter : ScriptedImporter
{
    ...
    bool ParseCubeData(AssetImportContext ctx, out int lutSize, out Color[] pixels)
    {
        ...
        for (int i = 0; true; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                if (!float.TryParse(row[j], NumberStyles.Float, CultureInfo.InvariantCulture.NumberFormat,
                    out var d))
                    return Error($"Invalid data on line {i}");
